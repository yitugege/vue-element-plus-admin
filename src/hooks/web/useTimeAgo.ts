...
const TIME_AGO_MESSAGE_MAP: {
  ...
  es: {
    ...
    past: (n) => (n.match(/\d/) ? `${n} atrás` : n),
    future: (n) => (n.match(/\d/) ? `en ${n}` : n),
    month: (n, past) => (n === 1 ? (past ? 'mes pasado' : 'próximo mes') : `${n} meses`),
    year: (n, past) => (n === 1 ? (past ? 'año pasado' : 'próximo año') : `${n} años`),
    day: (n, past) => (n === 1 ? (past ? 'ayer' : 'mañana') : `${n} días`),
    week: (n, past) => (n === 1 ? (past ? 'semana pasada' : 'próxima semana') : `${n} semanas`),
    hour: (n) => `${n} hora${n > 1 ? 's' : ''}`,
